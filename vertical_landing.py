...
The drone has already left the train and has just finished the data acquisition. The train has a varying velocity around 
40 km/h and it is distant around 900 m from the drone. The drone has a very tiny velocity along x and y axis. The top of
...
for i in range(0, len(t_consensus) - 1):
    ...
    """
    term1 = np.tanh(np.array([xd[0, i] - xt[0, i] - 153, xd[1, i] - xt[1, i] - 31.415160951928783,
                              xd[2, i] - xt[2, i] - 1,
    ...
    zeta_dot = np.array([[xt[3, i+1] - xt[3, i]],
                         [xt[4, i+1] - xt[4, i]],
                         [xt[5, i+1] - xt[5, i]],
                         [0],
                         [0],
                         [0]])
    a_drone = 1 / k * zeta_dot - (1 / k) * np.dot(Kr, term1) - (1 / k) * np.dot(Kv, term2)    # Accelerations
...
# print(xt[3, -1])    # Train state after consensus
# print(xt[3, -1])    # Train state after consensus
xt_init = xt[0, -1]
yt_init = xt[1, -1]
...
# Final condition
zbar = np.array([xd[0, -1], xd[1, -1], 3, 0, 0, 0, 0, 0, 0, 0, 0, 0])
ubar = np.array([drone.m * g, 0, 0, 0])

# Parameters for the LQR Algorithm
R = np.identity(4) * 2
Q = np.identity(12) * 550
horizon_length = 1000
...
ax1.text(16500, - 300, 3, s='Train', fontweight='bold')
...
ax1.text(500, 1200, 3.9, s='Drone', fontweight='bold')
cmap_v = mpl.cm.turbo
